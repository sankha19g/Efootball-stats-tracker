...
const PlayerDetailsModal = ({ player, players = [], onClose, onUpdate, onSelectPlayer, onAddToCompare, initialEditMode = false, settings, showAlert, showConfirm, onUpdateBadge, userId }) => {
    ...
    // Stats Context Menu State
    const [contextMenu, setContextMenu] = useState(null);
    const [menuStats, setMenuStats] = useState({ matches: 0, goals: 0, assists: 0 });
    const contextMenuRef = useRef(null);

    // Rating Context Menu State
    const [ratingContextMenu, setRatingContextMenu] = useState(null);
    ...
    const featuredPackPlayers = useMemo(() => {
        const packName = (formData.featured || formData['Featured Players'] || player.featured || player['Featured Players'] || '').trim();
        if (!packName) return [];
        return players.filter(p => {
            const pPack = (p.featured || p['Featured Players'] || '').trim();
            return pPack.toLowerCase() === packName.toLowerCase();
        });
    }, [formData.featured, formData['Featured Players'], player.featured, player['Featured Players'], players]);
    ...
    const handleStatsContextMenu = (e) => {
        e.preventDefault();
        setMenuStats({
    ...
    const handleRatingContextMenu = (e) => {
        e.preventDefault();
        setEditRatingValue(player.rating || 0);
    ...
    const handleTouchStart = (e, type) => {
        ...
        // Set a timer for 600ms (standard hold duration)
    ...
    const handleTouchMove = (e) => {
        ...
        // Cancel long press if the user moves/scrolls significantly
        ...
        <div className={`fixed inset-0 bg-[#0a0a0c] z-[300] overflow-y-auto md:overflow-hidden flex flex-col ${settings?.highPerf ? '' : 'animate-fade-in'}`}>
                                        ...
                                        <section>
                                            ...
                                            <div className="grid grid-cols-1 gap-4 mt-3">
                                                ...
                                                <div className="grid grid-cols-1 gap-3">
                                                    <div className="flex flex-col gap-1.5">
                                                        <label className="text-[10px] font-black uppercase tracking-widest opacity-40 ml-1">Secondary / Trained Positions</label>
                                                        <input type="text" name="secondaryPosition" value={formData.secondaryPosition || ''} onChange={handleChange} placeholder="e.g. SS, AMF, DMF" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm font-bold text-white focus:border-ef-accent focus:outline-none" />
                                                    </div>
                                                </div>
                                            </div>
                                        </section>
                                        ...
                                        <div className={`flex-1 flex flex-col transition-opacity duration-150 md:overflow-y-auto no-scrollbar ${modalPage === 0 ? 'opacity-100 relative' : 'opacity-0 pointer-events-none absolute inset-0'} `}>
                                            ...
                                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                                                {[
                                                    ...
                                                    <button
                                                        ...
                                                        onTouchStart={(e) => handleTouchStart(e, 'stats')}
                                                        onTouchEnd={handleTouchEnd}
                                                        onTouchMove={handleTouchMove}
                                                        className="bg-[#111111] border border-white/5 rounded-[0.5rem] p-5 flex flex-col items-start justify-center shadow-xl text-left w-full  focus:outline-none "
                                                    >
                                            ...
                                            {playerAwards.length > 0 && (
                                                <div className="mt-4 bg-[#0e0e10] border border-white/5 rounded-[1.5rem] p-5 shadow-xl transition-all">
                                                    {/* Header */}
                                                    <div
                                                        className="flex items-center justify-between cursor-pointer select-none group/awards"
                                                            ...
                                                            <div className="flex items-center gap-3 mb-6 flex-wrap">
                                                                <div className="flex items-center gap-1 bg-white/5 p-1 rounded-xl border border-white/5">
                                                                    {availableAwardPositions.map(pos => (
                                                                        <button
                                                                            key={pos}
                                                                            onClick={(e) => { e.stopPropagation(); setAwardFilter(pos); }}
                                                                            className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${awardFilter === pos
                                                                    ...
                                                                    <li key={i} className="flex items-start gap-3 group">
                                                                        ...
                                                                        <div className="flex flex-1 items-start justify-between gap-2 min-w-0">
                                                                            <span className="text-[12px] font-medium text-white/70 leading-snug font-inter">
                                                                                <span className={`font-black ${aw.rank === 1 ? 'text-yellow-400' :
            ...
            {contextMenu && (
                <div
                    ...
                    className="bg-[#121216]/95 backdrop-blur-xl border border-white/10 rounded-xl p-4 shadow-2xl w-60 animate-in fade-in zoom-in-95 duration-100"
                >
                    ...
                    <div className="space-y-3">
                        <div className="flex flex-col gap-1">
                            ...
                            <input
                                type="number"
                                value={menuStats.matches}
                                onChange={(e) => setMenuStats(prev => ({ ...prev, matches: parseInt(e.target.value) || 0 }))}
                                className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-xs text-white focus:border-ef-accent focus:outline-none transition-all font-mono font-bold"
                            />
                        </div>
                        <div className="flex flex-col gap-1">
                            <label className="text-[9px] font-black uppercase tracking-widest text-white/40 block">
                            ...
                            <input
                                type="number"
                                value={menuStats.goals}
                                onChange={(e) => setMenuStats(prev => ({ ...prev, goals: parseInt(e.target.value) || 0 }))}
                                className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-xs text-white focus:border-ef-accent focus:outline-none transition-all font-mono font-bold"
                            />
                        </div>
                        <div className="flex flex-col gap-1">
                            ...
                            <input
                                type="number"
                                value={menuStats.assists}
                                onChange={(e) => setMenuStats(prev => ({ ...prev, assists: parseInt(e.target.value) || 0 }))}
                                className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-xs text-white focus:border-ef-accent focus:outline-none transition-all font-mono font-bold"
                            />
                        </div>
                        <div className="flex gap-2 pt-2 border-t border-white/5">
                            <button
                                ...
                                className="flex-1 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-white/60 hover:text-white text-[10px] font-black uppercase tracking-widest transition-all"
                            >
                                Cancel
                            </button>
                            <button
                                ...
                                onClick={() => {
                                    const updatedFields = {
                                        matches: menuStats.matches,
                                        goals: menuStats.goals,
                                        assists: menuStats.assists
                                    };
                                ...
                                className="flex-1 py-1.5 bg-ef-accent hover:brightness-110 text-ef-dark rounded-lg text-[10px] font-black uppercase tracking-widest transition-all shadow-lg shadow-ef-accent/20"
                            >
                                Save
                            </button>
                        </div>
            ...
            {ratingContextMenu && (
                <div
                    ref={ratingContextMenuRef}
                    style={{
                        position: 'fixed',
                        top: `${ratingContextMenu.y}px`,
                        left: `${ratingContextMenu.x}px`,
                        zIndex: 9999
                    }}
                    className="bg-[#121216]/95 backdrop-blur-xl border border-white/10 rounded-xl p-4 shadow-2xl w-60 animate-in fade-in zoom-in-95 duration-100"
                >
                    ...
                    <div className="space-y-3">
                        <div className="flex flex-col gap-1">
                            ...
                            <input
                                ...
                                onChange={(e) => setEditRatingValue(parseInt(e.target.value) || 0)}
                                className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-xs text-white focus:border-ef-accent focus:outline-none transition-all font-mono font-bold"
                            />
                        </div>
                        <div className="flex gap-2 pt-2 border-t border-white/5">
                            <button
                                type="button"
                                onClick={() => setRatingContextMenu(null)}
                                className="flex-1 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-white/60 hover:text-white text-[10px] font-black uppercase tracking-widest transition-all"
                            >
                                Cancel
                            </button>
                            <button
                                type="button"
                                onClick={() => {
                                    onUpdate(player._id, { rating: editRatingValue }, false);
                                    setFormData(prev => ({
                                        ...prev,
