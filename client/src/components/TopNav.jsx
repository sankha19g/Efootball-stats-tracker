                ...
                <div className="flex items-center gap-4">
                    <button
                        onClick={() => {
                            if (isOpen) {
                                setIsOpen(false);
                            } else if (isSubView && !['leaderboard', 'squad-builder', 'quick-stats', 'squad-db', 'badges', 'activity-log', 'settings', 'profile-stats', 'brochure', 'screenshots', 'quick-links'].includes(view)) {
                                setView('list');
                            } else {
                                setIsOpen(true);
                            }
                    ...
                    >
                        ...
                        ) : (isSubView && !['leaderboard', 'squad-builder', 'quick-stats', 'squad-db', 'badges', 'activity-log', 'settings', 'profile-stats', 'brochure', 'screenshots', 'quick-links'].includes(view)) ? (
                                    ...
                                    {selectedIds.size > 0 && (
                                        <>
                                            ...
                                            <button
                                                ...
                                                className="h-10 px-4 rounded-xl bg-red-600/20 text-red-500 hover:text-white hover:bg-red-600 text-[10px] font-black uppercase tracking-widest border border-red-500/20 hover:border-red-600 transition-all flex items-center gap-2"
                                            >
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar pb-1 w-full">
                    {isSelectionMode ? (
                        <>
                            <button
                                onClick={() => { setIsSelectionMode(false); setSelectedIds(new Set()); }}
                                className="px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-[10px] font-black uppercase flex items-center justify-center shrink-0"
                                title="Cancel Selection"
                            >
                            ...
                            {selectedIds.size > 0 ? (
                                <>
                                    <button
                                        onClick={() => setShowBulkEdit(true)}
                                        className="flex-1 py-2 rounded-lg bg-ef-blue text-white text-[10px] font-black uppercase flex items-center justify-center gap-1.5"
                                    >
                                        <Edit3 className="w-3 h-3" /> Edit ({selectedIds.size})
                                    </button>
                                    <button
                                        onClick={handleBulkDelete}
                                        className="flex-1 py-2 rounded-lg bg-red-600 text-white text-[10px] font-black uppercase flex items-center justify-center gap-1.5"
                                    >
                                        <Trash2 className="w-3 h-3" /> Delete ({selectedIds.size})
                                    </button>
                                </>
                            ) : (
                                <div className="flex-1 text-[9px] font-bold text-white/40 uppercase tracking-wider flex items-center justify-center py-2">
                                    Select cards to edit/delete
                                </div>
                            )}
                        </>
                    ) : (
                        <>
                            ...
                            {user && view === 'list' && (
                                <button
                                    onClick={() => setIsSelectionMode(true)}
                                    className="flex-1 py-2 rounded-lg bg-white/5 border border-white/10 text-white/40 text-[10px] font-black uppercase flex items-center justify-center gap-2"
                                >
