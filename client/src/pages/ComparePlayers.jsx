...
const ComparePlayers = ({ players, onPlayerClick, settings, initialSelectedIds, onIdsChange }) => {
    ...
    const ComparisonRow = ({ label, values, suffix = '', isNumeric = true, reverse = false, showWinner = true }) => {
        ...
        const maxVal = validNumericValues.length > 0 ? Math.max(...validNumericValues) : -Infinity;
        const minVal = validNumericValues.length > 0 ? Math.min(...validNumericValues) : -Infinity;

        const isGreater = (val, idx) => {
            ...
            return reverse ? (num === minVal) : (num === maxVal);
        };

        const isLesser = (val, idx) => {
            if (val === null || val === undefined || val === '-' || !showWinner) return false;
            if (validNumericValues.length < 2) return false;
            if (maxVal === minVal) return false;
            const num = numericValues[idx];
            return reverse ? (num === maxVal) : (num === minVal);
        };
        ...
        return (
            <div className="py-2 border-b border-[var(--border-subtle)] group hover:bg-[var(--bg-hover)] transition-all px-4">
                <div className="flex flex-col gap-1.5">
                    ...
                    <div className={`grid gap-4 items-center ${selectedIds.length === 2 ? 'grid-cols-2' :
                        ...
                        {values.map((v, i) => (
                            <div key={i} className="flex flex-col min-w-0">
                                ...
                                <div className="flex items-center gap-1.5">
                                    <div className={`font-mono transition-all duration-120 flex items-baseline gap-0.5 ${isGreater(v, i) ? 'text-green-400 font-bold' :
                                        isLesser(v, i) ? 'text-red-400 font-bold' :
                                    ...
                                    {isGreater(v, i) && <div className="w-1 h-1 rounded-full bg-green-400 animate-pulse" />}
                                    {isLesser(v, i) && <div className="w-1 h-1 rounded-full bg-red-400 animate-pulse" />}
                                </div>
                            </div>
                        ))}
                    </div>
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 md:px-8 pt-0 pb-32 atlas-reveal">
            ...
            {/* Selection Slots */}
            <div className={`grid gap-4 mb-12 ${selectedIds.length === 2 ? 'grid-cols-1 md:grid-cols-2' :
                ...
                {selectedIds.map((id, idx) => {
                    ...
                    const playerImage = player ? (
                        settings?.preferredImageSource === 3
                            ? ((player.playerId || player.pesdb_id || player.id || player.ID)
                                ? `https://efimg.com/efootballhub22/images/player_cards/${player.playerId || player.pesdb_id || player.id || player.ID}_l.png`
                                : (player.image || player.image2))
                            : (settings?.preferredImageSource === 2
                                ? (player.image2 || player.image)
                                : (player.image || player.image2))
                    ) : '';
                    ...
                    return (
                        <div key={idx} className={`atlas-card !overflow-visible relative group transition-all duration-200 ${!id ? 'border-dashed border-[var(--border-strong)]' : ''} ${searchTerms[idx] ? 'z-[100]' : 'z-10'}`}>
                            {/* Render header ONLY when slot is empty */}
                            {!id && (
                                <div className="atlas-card-header justify-between bg-[var(--bg-raised)]">
                                    <div className="flex items-center gap-2">
                                        <span className="atlas-dot atlas-dot-warning"></span>
                                        <span className="atlas-panel-title">PLAYER {idx + 1}</span>
                                    </div>
                            ...
                            {/* Slot remove button overlay for populated card container (to allow slot deletion) */}
                            {id && selectedIds.length > 2 && (
                                <button
                                    onClick={() => removeSlot(idx)}
                                    className="absolute top-4 right-4 text-white/35 hover:text-white/80 transition-all hover:scale-110 z-30"
                                    title="Remove Slot"
                                >
                            ...
                            <div className={`${id ? 'p-6 bg-black/40' : 'p-4 bg-[var(--bg-surface)]'}`}>
                                {id ? (
                                    <div className="flex flex-col items-center space-y-4">
                                        {/* Player slot header label */}
                                        <span className="text-[12px] font-bold text-zinc-400 uppercase tracking-widest">
                                            Player {idx + 1}
                                        </span>

                                        {/* Player Card Image Wrapper */}
                                        <div className="relative w-44 aspect-[7/10]">
                                            <div className="w-full h-full bg-black/80  border border-white/10 overflow-hidden shadow-2xl transition-all group-hover:border-white/20">
                                                {playerImage ? (
                                                    <img
                                                        src={playerImage}
                                                        alt={player.name}
                                                    ...
                                                    <div className="w-full h-full flex flex-col items-center justify-center">
                                                        ...
                                                        <span className="text-[8px] font-black uppercase tracking-[0.2em] text-white/20 mt-2">No Visual</span>
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            <button
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    setPlayerId(idx, null);
                                                }}
                                                className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-black text-white border border-white flex items-center justify-center shadow-lg hover:scale-110 active:scale-95 transition-all z-20"
                                                title="Clear player"
                                            >
                                            ...
                                            <div className="flex items-center justify-center gap-3 pt-1">
                                                {(player.logos?.club || player.club_badge_url) && (
                                                    <img
                                                        ...
                                                        alt=""
                                                        className="w-6 h-6 object-contain filter drop-shadow-md brightness-110"
                                                    />
                                                )}
                                                {(player.logos?.country || player.nationality_flag_url) && (
                                                    <div className="w-7 h-4.5 overflow-hidden border border-white/10 shadow-sm rounded-sm shrink-0">
                                                        <img
            ...
            {activeNPlayers.length >= 2 ? (
                <div className="atlas-card !border-white/5 shadow-2xl">
                    ...
                    <div className="divide-y divide-white/5 bg-black">
                        ...
                        {/* Positions Display */}
                        <div className={`grid divide-x divide-white/5 border-t border-white/5 ${selectedIds.length === 2 ? 'grid-cols-2' :
                            selectedIds.length === 3 ? 'grid-cols-3' :
                                'grid-cols-2 lg:grid-cols-4'
                            }`}>
                            {nPlayers.map((p, idx) => (
                        ...
                        {getSkillAnalysis && (
                            <div className="p-10 bg-black border-t border-white/5">
                                ...
                                <div className="space-y-12">
                                    {/* Common Skills */}
                                    <div className=" bg-zinc-950 p-8 rounded-[var(--radius-lg)] border border-white/5 shadow-2xl relative overflow-hidden group">
                                    ...
                                    <div className="space-y-5">
                                        ...
                                        <div className={`grid gap-5 ${selectedIds.length === 2 ? 'grid-cols-2' :
                                            selectedIds.length === 3 ? 'grid-cols-3' :
                                                'grid-cols-2 lg:grid-cols-4'
                                            }`}>
                                            {getSkillAnalysis.unique.map((item, idx) => (
                                                <div key={idx} className="space-y-5 p-6 rounded-[var(--radius-lg)] bg-zinc-950/50 border border-white/5 hover:border-white/10 hover:bg-zinc-950 transition-all group">
                                                    ...
                                                    <div className="flex flex-wrap gap-1.5">
                                                        {item.skills.map(s => (
                                                            <span key={s} className="px-3 py-1 rounded-sm bg-zinc-900 border border-white/10 text-[10px] font-bold text-zinc-300 hover:border-[var(--accent-highlight)]/30 transition-colors uppercase tracking-tight">{s.replace(/\s+/g, '')}</span>
                                                        ))}
